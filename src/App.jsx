...
const STEPS_AM=[
  {id:"veille",time:"8h30",to:"9h30",title:"Reprise de la veille",desc:"Reprends les leads non contactés et les devis en attente d'hier.",color:"#60A5FA",emoji:"🚦",
    actions:["Relancer les leads sans réponse","Suivre les devis en cours","Vérifier les tickets Zendesk ouverts"],
    tools:[{l:"Zendesk",u:"https://rubix.zendesk.com"}]},
  {id:"leads",time:"9h30",to:"12h00",title:"Traitement des leads",desc:"Vérifier MR 03 dans Qliksense, puis traiter dans Rubix Go (lien depuis Qlik).",color:"#FBBF24",emoji:"🏎️",
    tip:"13% des 66FFF = comptes Gesfi existants. Vérifier Qliksense MR 03 avant création.",
    actions:["Vérifier clients MR 03 dans Qliksense","Traiter les nouveaux leads (via Rubix Go depuis Qlik)","Transférer les clients connus vers agences","Appels nouveaux clients + création 66FFF","Gérer les redirections si erreur"],
    tools:[{l:"Qliksense MR03",u:"https://qliksense.rubix.com"}]},
];
const STEPS_PM=[
  {id:"paniers",time:"13h30",to:"14h30",title:"Relance paniers abandonnés",desc:"Clients connus vers agence/ATE. 66FFF optin vérifié vers relance.",color:"#FB923C",emoji:"🛞",
    tip:"RGPD : vérifier l'optin avant tout appel commercial 66FFF.",
    actions:["Clients connus (non 66FFF) vers agences/ATE","66FFF optin vérifié vers relance appel"],
    tools:[{l:"Rubix Go",u:"https://go.rubix.com"}]},
  {id:"devis",time:"14h30",to:"17h00",title:"Réponse aux devis",desc:"Demandes fournisseurs, rédaction devis, contrôle commandes.",color:"#34D399",emoji:"🏁",
    actions:["Demandes aux fournisseurs","Rédaction des devis","Contrôle commandes 66FFF (AT/AV)"],
    tools:[{l:"Rubix Go",u:"https://go.rubix.com"}]},
];
const ALL_STEPS=[...STEPS_AM,...STEPS_PM];
const ABS=[{id:"none",l:"Disponible",e:"",c:"#00D26A"},{id:"formation",l:"Formation",e:"🎓",c:"#A855F7"},{id:"reunion",l:"Réunion",e:"🤝",c:"#60A5FA"},{id:"deplacement",l:"Déplacement",e:"🚗",c:"#F59E0B"},{id:"off",l:"Off",e:"🏖️",c:"#94A3B8"}];
const CANAUX=["Email","Chat","Téléphone"];
...
function buildAT(dd){var p=[];ALL_STEPS.forEach(function(st){var sd=(dd.steps||{})[st.id];if(sd&&sd.comment&&sd.comment.trim())p.push("• "+st.title+" : "+sd.comment.trim());});return p.join("\n");}
...
function getProMsg(pct,h,step,checkedAct,totalAct,isOff){
  if(pct===100)return"Objectif atteint. Toutes les actions du jour sont bouclées.";
  if(isOff)return"Journée off. Bon repos, la reprise n'en sera que meilleure.";
  if(step){var stepMsgs={
    ...
    leads:["Phase de traitement : chaque lead qualifié est une opportunité de CA.","Concentration maximale sur les MR 03, c'est là que se fait la différence.","Le volume se construit maintenant, lead après lead.","Qualifiez, transférez, créez. Le pipeline se remplit."],
    paniers:["Les paniers abandonnés sont du CA à portée de main.","Un appel bien placé peut transformer un panier en commande.","Vérifiez l'optin, puis décrochez. Chaque relance compte."],
    devis:["Les devis envoyés rapidement ont un meilleur taux de conversion.","Précision et réactivité sur les devis, c'est ce qui fidélise.","Chaque devis est une promesse de valeur pour le client."]
  };if(stepMsgs[step]){var sm=stepMsgs[step];return sm[Math.floor(h+totalAct)%sm.length];}}
  if(pct>=80){var msgs=["Plus que quelques actions pour boucler la journée.","Vous y êtes presque, ne lâchez rien.","Dernière ligne droite, l'objectif est en vue."];return msgs[Math.floor(h+checkedAct)%msgs.length];}
  if(pct>=50){var msgs2=["Bonne progression, la moitié du chemin est faite.","Rythme solide, continuez comme ça.","Cap dépassé, maintenez l'effort."];return msgs2[Math.floor(h+checkedAct)%msgs2.length];}
  var morning=["Début de journée, posez les bases d'une session productive.","Les premières heures donnent le ton, capitalisez dessus.","Matinée lancée, gardez le rythme.","Bonne dynamique matinale, continuez sur cette trajectoire."];
  var midday=["Pause méritée, vous reprendrez avec plus de clarté.","Mi-journée passée, le plus dur est fait."];
  var afternoon=["L'après-midi est le moment idéal pour closer les dossiers ouverts.","Restez focus sur les priorités, la fin de journée approche.","Dernier tiers de la journée, finissez fort.","Sprint final, chaque action complétée est un pas de plus."];
  var evening=["Pensez à finaliser votre topo avant de quitter.","Bonne fin de journée, sécurisez vos avancées.","Il est temps de consolider et préparer demain."];
...
function ReportingForm({day,onAdd}){
  ...
  return(
    <div style={{background:"#fff",borderRadius:16,padding:20,border:"1.5px solid #E2E8F0",boxShadow:"0 1px 3px rgba(5,30,80,.04)"}}>
      <div style={{fontSize:15,fontWeight:800,color:"#051E50",marginBottom:16,fontFamily:HD}}>Nouvelle entrée</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
        ...
        <div><Lbl>Canal</Lbl><div style={{display:"flex",gap:3}}>{CANAUX.map(function(c){return <button key={c} onClick={function(){upd("canal",c);}} style={{flex:1,padding:"8px 2px",borderRadius:8,border:"1.5px solid "+(e.canal===c?"#60A5FA":"#E2E8F0"),background:e.canal===c?"#EFF6FF":"#fff",fontSize:10,fontWeight:700,cursor:"pointer",color:e.canal===c?"#1E40AF":"#94A3B8",transition:"all .15s"}}>{c==="Téléphone"?"📞":c==="Email"?"✉️":"💬"}</button>;})}</div></div>
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginTop:10}}>
        <div><Lbl>CA commandé</Lbl><Inp value={e.caCommande} onChange={function(v){upd("caCommande",v);}} placeholder="€"/></div>
        <div><Lbl>Valeur Devis</Lbl><Inp value={e.valeurDevis} onChange={function(v){upd("valeurDevis",v);}} placeholder="€"/></div>
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:5,marginTop:12}}>
        ...
        <Chk label="Création Gesfi" checked={e.creationGesfi} onChange={function(v){upd("creationGesfi",v);}}/> 
...
function ReportingTable({entries,onDelete,onReorder}){
  ...
  if(!entries||!entries.length)return <div style={{textAlign:"center",padding:"32px",color:"#94A3B8",fontSize:13}}>Aucune entrée ce jour</div>;
  var cols=[{k:"gesfi",l:"Gesfi",w:70},{k:"client",l:"Client",w:100},{k:"canal",l:"Canal",w:60},{k:"typeDemande",l:"Demande",w:80},{k:"existantSiret",l:"Siret",w:40,b:1},{k:"ca12mois",l:"CA12m",w:40,b:1},{k:"rapportRGo",l:"RGo",w:35,b:1},{k:"creationGesfi",l:"Créa.",w:35,b:1},{k:"agence",l:"Agence",w:70},{k:"rubixClub",l:"Club",w:35,b:1},{k:"caCommande",l:"CA cmd",w:60},{k:"valeurDevis",l:"Devis€",w:60},{k:"mailLeads",l:"Mail",w:80},{k:"actionsDavid",l:"Actions",w:80},{k:"relanceDevis",l:"Rel.",w:30,b:1},{k:"ouvertureAcces",l:"Acc.",w:30,b:1},{k:"commentaires",l:"Com.",w:80}];
    ...
    <div style={{background:"#fff",borderRadius:14,overflow:"hidden",border:"1.5px solid "+(open?step.color+"44":"#E2E8F0"),boxShadow:open?"0 8px 32px "+step.color+"10":"0 1px 3px rgba(0,0,0,.03)",transition:"all .25s"}}>
      <div onClick={function(){setOpen(!open);}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"14px 18px",cursor:"pointer",background:open?"linear-gradient(135deg,"+step.color+"06,transparent)":"transparent"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <span style={{fontSize:16,color:"#CBD5E1",transform:open?"rotate(180deg)":"",transition:"transform .3s"}}>▾</span>
        </div>
      </div>
        ...
        <div style={{background:"#FAFBFF",borderRadius:12,padding:"4px 16px"}}>
          {step.actions.map(function(action,i){var isC=!!(sd.checks&&sd.checks[i]);return(
            <div key={i} onClick={function(ev){ev.stopPropagation();onToggle(i);}} style={{display:"flex",alignItems:"center",gap:14,padding:"12px 0",borderBottom:i<total-1?"1px solid #EDF2F7":"none",cursor:"pointer"}}>
              <div style={{width:26,height:26,borderRadius:8,border:"2px solid "+(isC?"#00D26A":"#D1D5DB"),background:isC?"linear-gradient(135deg,#00D26A,#059669)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all .2s",boxShadow:isC?"0 2px 10px rgba(0,210,106,.25)":"none"}}>{isC&&<span style={{color:"#fff",fontSize:13,fontWeight:900}}>✓</span>}</div>
              <span style={{fontSize:14,color:isC?"#CBD5E1":"#0F172A",textDecoration:isC?"line-through":"none",flex:1,lineHeight:1.5}}>{action}</span>
            </div>);})}
        </div>
        <div style={{marginTop:14}}><Lbl>Commentaire (→ topo)</Lbl><TA2 value={sd.comment||""} onChange={onComment} placeholder="Notes, chiffres clés..." rows={2}/></div>
      </div>)}
    </div>);
}
...
function AbsPill({label,value,onChange,disabled}){
  ...
  return(<div style={{position:"relative"}}><Lbl>{label}</Lbl>
    <button onClick={function(){if(!disabled)setOpen(!open);}} style={{display:"flex",alignItems:"center",gap:6,padding:"9px 14px",borderRadius:10,border:"1.5px solid "+(isA?cur.c+"55":"#E2E8F0"),background:isA?cur.c+"08":"#fff",cursor:disabled?"default":"pointer",fontSize:13,fontWeight:700,color:isA?cur.c:"#00D26A",width:"100%",opacity:disabled?.5:1}}>
      {isA?<span>{cur.e}</span>:<span>✓</span>}<span style={{flex:1,textAlign:"left"}}>{cur.l}</span>{!disabled&&<span style={{color:"#CBD5E1"}}>▾</span>}
    </button>
...
export default function App(){
  ...
  useEffect(function(){setState(ld()||initState());},[]);
  useEffect(function(){if(state)sv(state);},[state]);
  useEffect(function(){
    try{document.title="Pôle Acquisition · Rubix";}catch(e){}
  ...
  function addEntry(e){setDD(function(d){return{...d,entries:[...(d.entries||[]),e]};});flash("Entrée ajoutée ✓");}
  ...
  var h2=new Date().getHours();var greet=h2<12?"Bonjour David":h2<14?"Bon appétit David":h2<18?"Bon après-midi David":"Bonne soirée David";
  ...
  function nextDay(){var nxt=nextWorkday(day);if(nxt>todayStr())return;setDay(nxt);}
  ...
  function doExportDayXLS(){if(!dayEntries.length){flash("Aucune entrée");return;}exportXLS(dayEntries,"Reporting_"+day+".xls");flash("Excel ✓");}
  function generateEmail(){
    var b="Bonjour,\n\nTopo du "+fmtD(day)+" (S"+wk(day)+")\nProgression : "+pct+"%\n";if(dayEntries.length)b+="Entrées reporting : "+dayEntries.length+"\n";b+="\n";
    ...
    b+="\nCordialement,\nDavid";window.open("mailto:?subject="+encodeURIComponent("[Pôle Acquisition] Topo "+fmtD(day))+"&body="+encodeURIComponent(b),"_blank");
  }

  var TABS=[{id:"home",l:"Accueil",e:"💻"},{id:"journee",l:"Journée",e:"📋"},{id:"reporting",l:"Appels, Emails, Chat",e:"📞"},{id:"mois",l:"Mois",e:"📅"}];

  function DayNav(){return(<div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
    <button onClick={prevDay} style={{background:"#fff",border:"1.5px solid #E2E8F0",color:"#475569",fontSize:22,width:42,height:42,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
    ...
    <button onClick={nextDay} style={{background:"#fff",border:"1.5px solid #E2E8F0",color:"#475569",fontSize:22,width:42,height:42,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
  </div>);}
    ...
    <div style={{fontFamily:FT,background:"#F0F2F8",color:"#0F172A",minHeight:"100vh",display:"flex",flexDirection:"column"}}>
      ...
      <header style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"0 28px",height:56,background:"linear-gradient(135deg,#051E50 0%,#0A2D6E 100%)",flexShrink:0,boxShadow:"0 2px 20px rgba(5,30,80,.2)"}}>
        <div style={{display:"flex",alignItems:"center",gap:16}}>
          ...
          <span style={{fontSize:16,fontWeight:800,color:"#FFD700",letterSpacing:.5}}>Pôle Acquisition</span>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <div style={{position:"relative"}}>
            <button onClick={function(){setSearchOpen(!searchOpen);if(!searchOpen)setTimeout(function(){try{document.getElementById("rubix-search").focus();}catch(e){}},100);}} style={{width:32,height:32,borderRadius:10,background:"rgba(255,255,255,.08)",border:"none",color:"#FFD700",fontSize:15,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>{"🔍"}</button>
            {searchOpen&&(<div style={{position:"absolute",top:42,right:0,width:380,background:"#fff",borderRadius:14,boxShadow:"0 16px 48px rgba(5,30,80,.2)",border:"1.5px solid #E2E8F0",zIndex:200,overflow:"hidden"}}>
              <div style={{padding:12}}>
                ...
                <div style={{fontSize:10,color:"#94A3B8",marginTop:6,lineHeight:1.5}}>Recherche dans vos entrées reporting, topos et commentaires saisis dans l'app.</div>
              </div>
              {search.trim().length>=2&&(<div style={{maxHeight:320,overflowY:"auto",padding:"0 12px 12px"}}>
                {(function(){var sr=getSearchResults();return sr.length===0?<div style={{padding:12,textAlign:"center",color:"#94A3B8",fontSize:13}}>Aucun résultat</div>:
                sr.map(function(r,i){
                  if(r.entry){return(<div key={i} onClick={function(){setDay(r.date);setTab("reporting");setSearchOpen(false);setSearch("");}} style={{padding:"10px 12px",borderRadius:10,cursor:"pointer",borderBottom:"1px solid #F1F5F9",fontSize:12}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{fontWeight:700,color:"#051E50"}}>{String(r.entry.client||r.entry.gesfi||"Entrée")}</span><span style={{fontFamily:MN,fontSize:10,color:"#94A3B8"}}>{r.date}</span></div>
        ...
        <div style={{flex:1,overflow:"auto",padding:"24px 32px 60px"}}>
          ...
          {tab==="home"&&(<div style={{animation:"fadeUp .35s ease",maxWidth:900}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                {activeSteps.map(function(st){var sd2=getStep(st.id);var ck=st.actions.filter(function(_,j){return(sd2.checks||{})[j];}).length;var tot=st.actions.length;var sp=tot?Math.round(ck/tot*100):0;var done=sp===100;
                  return(<div key={st.id} onClick={function(){setTab("journee");}} style={{cursor:"pointer",background:done?"#F0FDF4":"#FAFBFF",borderRadius:12,padding:"14px 12px",border:"1.5px solid "+(done?"#BBF7D0":st.color+"22"),textAlign:"center",transition:"all .15s"}}>
                    <div style={{fontSize:22,marginBottom:6}}>{done?"✅":st.emoji}</div>
            ...
            {dayEntries.length>0&&(<div style={{background:"#fff",borderRadius:16,padding:"20px 24px",border:"1.5px solid #E2E8F0",marginBottom:16}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:14,fontWeight:800,color:"#051E50"}}>Dernières entrées</div><button onClick={function(){setTab("reporting");}} style={{fontSize:12,fontWeight:700,color:"#60A5FA",background:"none",border:"none",cursor:"pointer"}}>Voir tout →</button></div>
              {dayEntries.slice(-3).reverse().map(function(e,i){return(<div key={e.id||i} style={{display:"flex",alignItems:"center",gap:12,padding:"10px 0",borderBottom:i<Math.min(dayEntries.length,3)-1?"1px solid #EDF2F7":"none"}}>
                ...
                <span style={{fontSize:13,fontWeight:600,color:"#0F172A",flex:1}}>{e.client||e.gesfi||"Entrée"}</span>
                {e.typeDemande&&<span style={{fontSize:12,color:"#94A3B8"}}>{e.typeDemande}</span>}
              </div>);})}
            </div>)}
            {pct===100&&!isFullOff&&(<div style={{background:"linear-gradient(135deg,#ECFDF5,#F0FDF4)",borderRadius:16,padding:"28px",textAlign:"center",border:"1.5px solid #BBF7D0",animation:"pop .5s ease"}}><div style={{fontSize:36}}>🏁</div><div style={{fontSize:20,fontWeight:900,color:"#00D26A",marginTop:8}}>Drapeau à damier !</div></div>)}
          </div>)}
          ...
          {tab==="journee"&&(<div style={{animation:"fadeUp .35s ease",maxWidth:720}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
              <button onClick={prevDay} style={{background:"#fff",border:"1.5px solid #E2E8F0",color:"#475569",fontSize:22,width:42,height:42,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
              <div style={{flex:1,textAlign:"center"}}><div style={{fontSize:18,fontWeight:700,color:"#0F172A",textTransform:"capitalize"}}>{fmtD(day)}</div><div style={{display:"flex",gap:8,justifyContent:"center",marginTop:5}}><span style={{fontFamily:MN,fontSize:11,fontWeight:600,background:"#F1F5F9",color:"#94A3B8",padding:"3px 10px",borderRadius:6}}>{"S"+wk(day)}</span>{day===todayStr()&&<span style={{fontSize:10,fontWeight:800,color:"#051E50",background:"#FFD700",padding:"3px 12px",borderRadius:6}}>Aujourd'hui</span>}</div></div>
              <button onClick={function(){setTab("reporting");}} style={{display:"flex",alignItems:"center",gap:6,padding:"10px 16px",borderRadius:12,border:"none",background:"#051E50",color:"#FFD700",fontSize:12,fontWeight:800,cursor:"pointer",boxShadow:"0 2px 10px rgba(5,30,80,.15)",whiteSpace:"nowrap"}}><span style={{fontSize:14}}>📞</span>{"+ Entrée"}{dayEntries.length>0&&<span style={{background:"#FFD700",color:"#051E50",fontSize:10,fontWeight:900,padding:"1px 6px",borderRadius:6,marginLeft:2}}>{dayEntries.length}</span>}</button>
              <button onClick={nextDay} style={{background:"#fff",border:"1.5px solid #E2E8F0",color:"#475569",fontSize:22,width:42,height:42,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>›</button>
            </div>
            {/* Relances du jour */}
            {(function(){var relances=[];Object.keys(state.days).forEach(function(k){(state.days[k].entries||[]).forEach(function(en){if(en.relanceDate===day)relances.push({from:k,entry:en});});});
              ...
              return(<div style={{background:"#FFF9E6",borderRadius:14,padding:16,border:"1.5px solid #FFD700",marginBottom:14}}>
                <div style={{fontSize:12,fontWeight:800,color:"#051E50",marginBottom:10}}>{"🔔 "+relances.length+" relance"+(relances.length>1?"s":"")+" prévue"+(relances.length>1?"s":"")+" aujourd'hui"}</div>
                {relances.map(function(r,i){return(<div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 0",borderBottom:i<relances.length-1?"1px solid #FFD70033":"none"}}>
                  <span style={{background:r.entry.canal==="Email"?"#DBEAFE":r.entry.canal==="Chat"?"#D1FAE5":"#FEF3C7",padding:"3px 8px",borderRadius:6,fontSize:10,fontWeight:700,color:"#051E50"}}>{r.entry.canal==="Téléphone"?"📞":r.entry.canal==="Email"?"✉️":"💬"}</span>
            ...
            <div style={{background:"#fff",borderRadius:14,padding:16,border:"1.5px solid #E2E8F0",marginBottom:14}}><div style={{fontSize:12,fontWeight:700,color:"#051E50",marginBottom:10}}>📅 Disponibilité</div><div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}><AbsPill label="Matin" value={dd.am||"none"} onChange={function(v){setDD(function(d2){return{...d2,am:v};});}}/><AbsPill label="Après-midi" value={dd.pm||"none"} onChange={function(v){setDD(function(d2){return{...d2,pm:v};});}}/></div></div>
            {isFullOff?(<div style={{background:"#fff",borderRadius:14,padding:"32px",textAlign:"center",border:"1.5px solid #E2E8F0"}}><div style={{fontSize:44}}>🏖️</div><div style={{fontSize:16,fontWeight:700,color:"#94A3B8",marginTop:10}}>Pas de créneau</div></div>):(
            <><div style={{display:"flex",flexDirection:"column",gap:10}}>{activeSteps.map(function(step){var sd2=getStep(step.id);return <StepCard key={step.id} step={step} sd={sd2} onToggle={function(i){toggleCheck(step.id,i);}} onComment={function(v){setStep(step.id,{comment:v});}}/>;})}
              {pct===100&&(<div style={{animation:"pop .5s ease",background:"linear-gradient(135deg,#ECFDF5,#FEF9C3)",borderRadius:14,padding:"28px",textAlign:"center",border:"1.5px solid #BBF7D0"}}><div style={{fontSize:40}}>🏁</div><div style={{fontSize:20,fontWeight:900,color:"#00D26A",marginTop:8}}>Drapeau à damier !</div></div>)}</div></>)}
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginTop:18}}>
              <div style={{background:"#fff",borderRadius:14,padding:18,border:"1.5px solid #E2E8F0",borderLeft:"5px solid #FFD700"}}><div style={{fontSize:14,fontWeight:700,color:"#051E50",marginBottom:10}}>📝 Topo</div>{autoTopo&&<div style={{background:"#FFFBEB",borderRadius:10,padding:"10px 14px",marginBottom:10,border:"1px solid #FDE68A",fontSize:12,color:"#0F172A",whiteSpace:"pre-line",lineHeight:1.7}}>{autoTopo}</div>}<TA2 value={dd.topo||""} onChange={function(v){setDD(function(d2){return{...d2,topo:v};});}} placeholder="Compléments..."/></div>
              <div style={{background:"#fff",borderRadius:14,padding:18,border:"1.5px solid #E2E8F0",borderLeft:"5px solid #A855F7"}}><div style={{fontSize:14,fontWeight:700,color:"#A855F7",marginBottom:10}}>❓ Questions</div><TA2 value={dd.questions||""} onChange={function(v){setDD(function(d2){return{...d2,questions:v};});}} placeholder="Questions manager..."/></div>
            </div>
            <div style={{background:"#fff",borderRadius:14,padding:18,marginTop:14,border:"1.5px solid #E2E8F0"}}><div style={{fontSize:14,fontWeight:700,color:"#051E50",marginBottom:10}}>Satisfaction</div><div style={{display:"flex",gap:8}}>{[{n:1,e:"😓",l:"Difficile"},{n:2,e:"😐",l:"Moyen"},{n:3,e:"🙂",l:"Correct"},{n:4,e:"😊",l:"Bien"},{n:5,e:"🔥",l:"Top !"}].map(function(item){return(<button key={item.n} onClick={function(){setDD(function(d2){return{...d2,satisfaction:d2.satisfaction===item.n?0:item.n};});}} style={{flex:1,padding:"10px 4px",borderRadius:12,border:"2px solid "+(dd.satisfaction===item.n?"#FFD700":"#E2E8F0"),background:dd.satisfaction===item.n?"linear-gradient(135deg,#FFFBEB,#FEF3C7)":"#fff",cursor:"pointer",textAlign:"center"}}><div style={{fontSize:24}}>{item.e}</div><div style={{fontSize:9,fontWeight:700,color:dd.satisfaction===item.n?"#051E50":"#94A3B8",marginTop:3}}>{item.l}</div></button>);})}</div></div>
            <div style={{display:"flex",gap:10,marginTop:18}}><button onClick={generateEmail} style={{flex:1,padding:"14px 28px",borderRadius:12,border:"none",background:"linear-gradient(135deg,#051E50,#0A2D6E)",color:"#FFD700",fontSize:14,fontWeight:800,cursor:"pointer"}}>✉️ Envoyer le topo à Denis</button></div>
          </div>)}
          ...
          {tab==="reporting"&&(<div style={{animation:"fadeUp .35s ease",maxWidth:1100}}>
            ...
            <div style={{background:"#fff",borderRadius:16,padding:18,marginTop:16,border:"1.5px solid #E2E8F0"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}><div style={{fontSize:16,fontWeight:800,color:"#051E50"}}>Entrées du jour</div>{dayEntries.length>0&&<button onClick={doExportDayXLS} style={{padding:"8px 16px",borderRadius:8,border:"1.5px solid #E2E8F0",background:"#fff",color:"#051E50",fontSize:11,fontWeight:700,cursor:"pointer"}}>📥 Jour .xls</button>}</div>
              <ReportingTable entries={dayEntries} onDelete={deleteEntry} onReorder={reorderEntries}/>
            </div>
            <button onClick={function(){var e2=allEntriesDayMonth();if(!e2.length){flash("Aucune entrée");return;}exportXLS(e2,"Reporting_Acquisition_"+dayMonthLabel.replace(/ /g,"_")+".xls");flash("Excel ✓");}} style={{width:"100%",padding:"14px",marginTop:16,borderRadius:12,border:"none",background:"linear-gradient(135deg,#051E50,#0A2D6E)",color:"#FFD700",fontSize:14,fontWeight:800,cursor:"pointer",boxShadow:"0 4px 20px rgba(5,30,80,.15)"}}>{"📥 Export Reporting mensuel · "+dayMonthLabel}</button>
          </div>)}

          {/* MOIS */}
          {tab==="mois"&&(<div style={{animation:"fadeUp .35s ease",maxWidth:900}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}>
              <button onClick={function(){setMo(function(p){return p-1;});}} style={{background:"#fff",border:"1.5px solid #E2E8F0",color:"#475569",fontSize:22,width:42,height:42,borderRadius:12,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>‹</button>
              ...
              <div style={{background:"#fff",borderRadius:16,padding:18,border:"1.5px solid #E2E8F0"}}>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:4}}>
                  {mDL.map(function(d2,i){
                    ...
                    if(future)return(<div key={i} style={{padding:"6px 4px",borderRadius:10,background:"#FAFBFF",textAlign:"center",opacity:.4}}><div style={{fontSize:13,fontFamily:MN,color:"#CBD5E1"}}>{parseInt(d2.split("-")[2])}</div></div>);
                    var dd2=state.days[d2]||emptyDay();var as2=getAS(dd2);var fo2=as2.length===0;var amR=ABS.find(function(r){return r.id===(dd2.am||"none");})||ABS[0];var isT=d2===todayStr();var isSel=d2===day;
                    ...
                    return(<button key={i} onClick={function(){setDay(d2);setTab("journee");}} style={{padding:"6px 4px",borderRadius:10,border:isSel?"2px solid #FFD700":isT?"2px solid #60A5FA":"1.5px solid #E2E8F0",background:isSel?"#FFD70012":"#fff",cursor:"pointer",textAlign:"center",position:"relative",overflow:"hidden"}}>
                      ...
                      {fo2?<div style={{fontSize:9,marginTop:2}}>{amR.e||"🏖️"}</div>:dayPct>0?<div style={{height:3,borderRadius:2,background:"#F1F5F9",margin:"4px 4px 0",overflow:"hidden"}}><div style={{height:"100%",background:dayPct===100?"#00D26A":"#FFD700",width:dayPct+"%",borderRadius:2}}/></div>:null}
                      {nE>0&&<div style={{position:"absolute",top:2,left:3,fontSize:8,fontFamily:MN,fontWeight:700,color:"#60A5FA"}}>{nE}</div>}
                      {hasRelance&&<div style={{position:"absolute",top:2,right:3,fontSize:8}}>{"🔔"}</div>}
                    </button>);
                  })}
                </div>
              </div>
              <div style={{background:"#fff",borderRadius:16,padding:18,border:"1.5px solid #E2E8F0"}}>
                <div style={{fontSize:15,fontWeight:800,color:"#051E50",marginBottom:16}}>Synthèse</div>
                {(function(){var prefix=mY+"-"+String(mM+1).padStart(2,"0");var ks=Object.keys(state.days).filter(function(k){return k.startsWith(prefix);});var worked=ks.filter(function(k){return getAS(state.days[k]).length>0;});
                  ...
                  var moods=[{n:1,e:"😓",c:"#FD8374"},{n:2,e:"😐",c:"#BBBBBC"},{n:3,e:"🙂",c:"#80A2FC"},{n:4,e:"😊",c:"#00C66A"},{n:5,e:"🔥",c:"#FFD700"}];
                  function moodFor(avg){if(avg<=0)return{e:"-",c:"#BBBBBC"};var m=moods[Math.min(Math.round(avg)-1,4)];return m||{e:"-",c:"#BBBBBC"};}
                  return(<div>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"12px 0",borderBottom:"1px solid #EDF2F7"}}><span style={{fontSize:13,color:"#475569",fontWeight:600}}>Complétion</span><span style={{fontFamily:MN,fontSize:20,fontWeight:900,color:"#00C66A"}}>{actRate+"%"}</span></div>
                    <div style={{marginTop:14}}>
                      <div style={{fontSize:12,fontWeight:700,color:"#051E50",marginBottom:10}}>Humeur par semaine</div>
                      {weeks.length===0?<div style={{fontSize:11,color:"#94A3B8"}}>Pas encore de données</div>:
            ...
            {(function(){
              ...
              var canaux=[{c:"Téléphone",bg:"#FFD700",cl:"#051E50"},{c:"Email",bg:"#60A5FA",cl:"#051E50"},{c:"Chat",bg:"#00D26A",cl:"#051E50"}];
              ...
              var badges=[
                {id:"first",label:"Premier 100%",icon:"⭐",need:1,desc:"1 journée complétée"},
                {id:"five",label:"Top 5",icon:"🏅",need:5,desc:"5 journées complétées"},
                {id:"ten",label:"Serial closer",icon:"🏆",need:10,desc:"10 journées complétées"},
                {id:"twenty",label:"Machine",icon:"💎",need:20,desc:"20 journées complétées"},
                {id:"fifty",label:"Légende",icon:"👑",need:50,desc:"50 journées complétées"},
                {id:"streak3",label:"Streak 3j",icon:"🔥",need:3,desc:"3 jours de suite",streak:true},
                {id:"streak5",label:"Streak 5j",icon:"⚡",need:5,desc:"5 jours de suite",streak:true},
                {id:"streak10",label:"Inarrêtable",icon:"🚀",need:10,desc:"10 jours de suite",streak:true}
              ];
              return(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18,marginTop:18}}>
                <div style={{background:"#fff",borderRadius:16,padding:18,border:"1.5px solid #E2E8F0"}}>
                  <div style={{fontSize:14,fontWeight:800,color:"#051E50",marginBottom:14}}>Répartition canaux</div>
                ...
                <div style={{background:"#fff",borderRadius:16,padding:18,border:"1.5px solid #E2E8F0"}}>
                  <div style={{fontSize:14,fontWeight:800,color:"#051E50",marginBottom:14}}>Série en cours</div>
                  <div style={{textAlign:"center",padding:"10px 0"}}>
                    <div style={{fontFamily:MN,fontSize:48,fontWeight:900,color:streak>=3?"#FFD700":"#051E50"}}>{String(streak)}</div>
                    <div style={{fontSize:12,fontWeight:700,color:"#94A3B8",marginTop:4}}>{"jour"+(streak>1?"s":"")+" consécutif"+(streak>1?"s":"")+" à 100%"}</div>
                ...
                <div style={{background:"#fff",borderRadius:16,padding:18,border:"1.5px solid #E2E8F0",gridColumn:"1 / -1"}}>
                  <div style={{fontSize:14,fontWeight:800,color:"#051E50",marginBottom:14}}>Récompenses</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10}}>
                    {badges.map(function(b){var unlocked=b.streak?(streak>=b.need):(perfectDays>=b.need);var progress=b.streak?Math.min(streak,b.need):Math.min(perfectDays,b.need);return(
                      <div key={b.id} style={{textAlign:"center",padding:"14px 8px",borderRadius:14,border:"1.5px solid "+(unlocked?"#FFD700":"#F1F5F9"),background:unlocked?"linear-gradient(135deg,#FFD70010,#FFD70020)":"#FAFBFF",opacity:unlocked?1:.5}}>
                        ...
                        {unlocked&&<div style={{fontSize:9,fontWeight:800,color:"#00D26A",marginTop:4}}>Débloqué</div>}
                      </div>);})}
                  </div>
                </div>
              </div>);
